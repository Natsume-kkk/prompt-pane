...
- 帮助：集中说明提示词操作、Git 状态、窗格操作、连接排障和左侧 Codex 显示排障。
...
如果左侧 Codex 的旧输出偶尔出现错位、断行或空白，而右侧提示词仍正常，这是 Codex 在 Windows 终端与 Zellij 中的滚动历史显示重排问题，不代表会话或提示词数据损坏。先聚焦左侧，调整一次窗口或窗格大小并滚动到底部；也可以按 Zellij 默认键 `Ctrl+p`、`f` 切换全屏后再切回，然后滚动到底部。自定义 Zellij 按键时以实际绑定为准。
