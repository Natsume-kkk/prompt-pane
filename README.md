...
每次 Codex 回答完成后，viewer 会更新当前会话的累计 token、上下文占用、5 小时／7 天限额、模型以及宽度允许时的 Git 分支状态。首次回答结束前，指标区显示 `Metrics: waiting for Codex response…`；已有指标中的未知字段显示 `--`，不用 `0` 代替未知值。默认 70/30 窗格保留 Token Tracker 同款 `█` 填充与 `░` 点阵进度条，窄栏拆行、宽栏合并。

内置主题为 `mocha`、`latte`、`frappe`、`macchiato`、`nord` 和 `dracula`，色值与语义映射来自 Token Tracker。默认 `auto` 会在可确认浅色背景时使用 `latte`，否则使用 `mocha`，但不会作为多余选项显示在主题列表中。帮助页列出按最长名称对齐的六套主题和对应色板，使用 `↑`／`↓` 实时预览并以 `Enter` 保存；`Theme preview · <name>` 固定语义示例会同步展示状态、普通／选中提示词、操作、指标、Git 和错误等内容的实际配色。帮助正文、未选中主题名称和普通提示词编号使用正常前景色。也可用 `PROMPT_PANE_THEME` 临时覆盖，`NO_COLOR` 会关闭颜色。
