...
内置主题为 `mocha`、`latte`、`frappe`、`macchiato`、`nord` 和 `dracula`，色值与语义映射来自 Token Tracker。默认 `auto` 会在可确认浅色背景时使用 `latte`，否则使用 `mocha`。帮助页直接列出对齐的主题名称和对应色板，使用 `↑`／`↓` 实时预览并以 `Enter` 保存；固定语义示例会同步展示状态、选中提示词、操作、指标、Git 和错误等内容的实际配色。也可用 `PROMPT_PANE_THEME` 临时覆盖，`NO_COLOR` 会关闭颜色。
