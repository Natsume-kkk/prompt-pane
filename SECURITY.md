...
任何会话边界都不得读取 App Server 或搜索 Codex 会话目录。`Stop` Hook 只允许按本次 Hook 提供的准确 `session_id` 与 `transcript_path` 逐行筛选当前会话的结构化 usage 元数据；路径为空时静默跳过。为识别用户中断，`UserPromptSubmit` 可以从同一准确 transcript 的当时文件末尾启动短期观察器；观察器只接受匹配 `session_id + turn_id` 的后续结构化 `turn_complete`／`turn_aborted` 类型与中止原因，终止后立即退出。两条路径都不得解析、传输或保留 prompt、回答、推理、工具内容或原始行，不得扫描目录、回退到最近会话文件或建立历史索引。准确 transcript 路径只允许作为 Hook／观察器的短期本地输入，不得进入 IPC、日志、错误、缓存或配置。
