...
func checkPlatform(goos, goarch string) error {
	if goos != "windows" || goarch != "amd64" {
		return fmt.Errorf("Prompt Pane %s supports Windows x64 only (current: %s/%s)", Version, goos, goarch)
	}
	return nil
}
...
func probePowerShellVersion(path string) (string, error) {
	...
	defer cancel()
	output, err := processutil.Output(ctx, path, []string{
		"-NoLogo", "-NoProfile", "-NonInteractive", "-Command",
		`[Console]::Out.Write($PSVersionTable.PSVersion.ToString())`,
	}, processutil.OutputOptions{Limit: 4 << 10})
	if err != nil {
		return "", err
	}
