...
5. Zellij 创建左 70%、右 30% 的工作区，左侧获得焦点且保持原有标题，右侧标题固定为 `PROMPTS`；保留窗格边框与鼠标 resize，边框在启动时跟随 Prompt Pane 已保存或环境覆盖的主题，但本次会话不显示边框悬停高亮和 resize 帮助文字，并将首次点击直通目标窗格，使用户不必先单独聚焦 viewer。
...
13. 用户可在帮助页预览并保存六套全局主题；主题配置只包含非敏感主题名称。Help 实时预览只更新 Prompt Pane 自身，外层 Zellij 边框在保存后的下一次工作区启动时更新。
